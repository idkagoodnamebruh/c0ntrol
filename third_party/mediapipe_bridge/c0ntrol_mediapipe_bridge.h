...
C0ntrolMpHandle* c0ntrol_mp_create(const char* model_path, int num_hands,
                                    float detection_confidence,
                                    float presence_confidence,
                                    float tracking_confidence,
                                    char* error, int error_size);
int c0ntrol_mp_detect_video(C0ntrolMpHandle* handle, const uint8_t* rgb,
                            int width, int height, int stride,
                            int64_t timestamp_ms, C0ntrolMpResult* result,
                            char* error, int error_size);
int c0ntrol_mp_close(C0ntrolMpHandle* handle, char* error, int error_size);
